...
1. Prompt1 角色设定与目标对齐
    - 为AI 设定角色 和 明确本次会话的目标。 将AI从一个 通用问答机器人,切换为一个专注的 领域专家 
```
...
- 需求故事变更意味着要再编译
- 修改源头 spec.md
- 规范变了，计划必须跟着变. 触发级联更新(plan/tasks). 可以使用 基于更新后的 spec.md 请重新评估 plan.md tasks.ms  标记出需求修改或新增的任务

- 最后是增量执行  现在,请执行新生成/变更的任务 T-00X 实现变更。

- 总 plan.md  目录结构
- task.md  1. 最佳实践目录架构  故事功能   ?
- 用户故事  验收标准  技术栈 任务列表  